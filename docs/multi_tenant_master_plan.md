# Multi-Tenant Architecture Master Plan
...
| :--- | :--- | :--- |
| **Stateless API Routing** | Vercel Serverless API routes (like `/api/consumers`) cannot read from `process.env.GOOGLE_SHEET_ID` anymore. | Store `cccCode` in the encrypted JWT session cookie. The backend extracts `cccCode` on each request and looks up the active sheet ID. |
...
### Authentication & Session Configuration Lookup
```mermaid
sequenceDiagram
    participant User
    participant App
...
- [ ] Create `lib/tenant-resolver.ts` to fetch and cache the `CCC_Registry` mappings (with a 5-minute cache to minimize master sheet reads).
- [ ] Add `cccCode` to the JWT payload in `lib/session.ts` and update session validation helpers.

### Phase 2: Dynamic API Routing
...
- [ ] Update all GET/POST API routes (`/api/consumers`, `/api/consumer-master`, `/api/dtr`, etc.) to:
